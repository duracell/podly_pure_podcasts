...
class Feed(db.Model):  # type: ignore[name-defined, misc]
    ...
    itunes_explicit = db.Column(db.Boolean, nullable=True) # True (yes) / False (no) / None (unknown)
    ...
    itunes_categories_json = db.Column(db.Text, nullable=True) # Store categories as JSON string
...
class Post(db.Model):  # type: ignore[name-defined, misc]
    ...
    itunes_explicit = db.Column(db.Boolean, nullable=True) # True (yes) / False (no) / None (unknown)
    itunes_episode_type = db.Column(db.String(20), nullable=True) # 'full', 'trailer', or 'bonus'
    itunes_image_url = db.Column(db.Text, nullable=True) # URL for episode-specific image
