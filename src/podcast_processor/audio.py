...
def trim_file(in_path: Path, out_path: Path, start_ms: int, end_ms: int) -> None:
    try:
        (
            ...
            # Select samples between the start and end times
            .filter("aselect", f"between(t,{start_ms/1000},{end_ms/1000})")
            # Reset timestamps to start from 0 for the clipped segment
            .filter("asetpts", "PTS-STARTPTS")
            # Output using default settings based on out_path extension
            .output(str(out_path))
            .overwrite_output()
            .run(quiet=True)
        )
    except ffmpeg.Error as e:
        print(f"Error trimming file '{in_path}' from {start_ms}ms to {end_ms}ms:")
        if e.stderr:
            print(e.stderr.decode())
        raise
