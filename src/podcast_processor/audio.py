...
def get_audio_duration_ms(file_path: str) -> Optional[int]:
    ...
    try:
        ...
        logger.error(
            f"An ffmpeg error occurred while trying to probe the file {file_path}: {stderr_output}"
        )
        return None
...
def _create_silent_file(out_path: str) -> None:
    ...
    logger.warning(
        f"No segments to keep/process for {out_path}. Outputting minimal silent file."
    )
    try:
        sample_rate = 44100
        (
...
def clip_segments_with_fade(
    ...
    try:
        ...
        if audio_duration_ms <= 0:
            ...
            logger.warning(
                f"Input file {in_path} has zero or negative duration. Outputting silent file."
            )
            return
        ...
        if not keep_intervals:
            _create_silent_file(out_path)
            logger.warning(
                f"No audio segment to keep for {in_path} based on ad times. Outputting silent file."
            )
            return
                    ...
                    if actual_fade_duration_ms > 0:
                        ...
                        fade_start_segment = (
                            in_stream.filter(
                                "aselect",
                                (
                                    f"between(t,{ad_start_ms / 1000.0},"
                                    f"{(ad_start_ms + actual_fade_duration_ms) / 1000.0})"
                                ),
                            )
                            .filter(
                                "afade",
                                type="out",
                                start_time=0,
                                duration=actual_fade_duration_ms / 1000.0,
                            )
                            .filter("asetpts", expr="PTS-STARTPTS")
                        )
        ...
        if not processed_streams:
            _create_silent_file(out_path)
            logger.warning(
                f"No streams generated after filtering for {in_path}. Outputting silent file."
            )
            return
        ...
        try:
            ...
            (
                ffmpeg.output(joined_stream, out_path)
                .overwrite_output()
            ...
            logger.error(
                f"FFmpeg error during final concatenation for {out_path}: {err_msg}"
            )
            raise

        except Exception as general_e:
            logger.error(
                f"General error during ffmpeg run for {out_path}: "
                f"Type={type(general_e).__name__}, Error={general_e}"
            )
            raise
        ...
        logger.error(
            f"Error in clip_segments_with_fade function for {in_path}: "
...
def trim_file(in_path: Path, out_path: Path, start_ms: int, end_ms: int) -> None:
    ...
    try:
        ...
        logger.error(
            f"Error trimming file '{in_path}' from {start_ms}ms to {end_ms}ms "
            f"into '{out_path}': {err_msg}"
        )
        raise
    except Exception as e:
...
def split_audio(
    ...
    if duration_ms <= 0:
        logger.warning(
            f"Audio file {audio_file_path} has zero or negative duration. No chunks generated."
        )
        return []
    ...
    try:
        ...
        if file_size <= 0:
            logger.warning(
                f"Audio file {audio_file_path} has zero size despite duration {duration_ms}ms. "
    ...
    if avg_bytes_per_ms <= 0:
        ...
        logger.warning(
            f"Could not calculate positive average bitrate for {audio_file_path}. "
            f"Treating as single chunk."
        )
    else:
    ...
    if chunk_duration_ms <= 0:
        logger.warning(
            f"Calculated chunk duration is non-positive ({chunk_duration_ms}ms) for "
            f"{audio_file_path} with target size {chunk_size_bytes}. Using minimum of 1ms."
        )
    ...
    logger.info(
        f"Splitting '{audio_file_path.name}' ({duration_ms}ms) into approx "
        f"{num_chunks} chunks of {chunk_duration_ms}ms..."
    )
    ...
    for i in range(num_chunks):
        ...
        if end_offset_ms <= start_offset_ms:
            logger.warning(
                f"Skipping chunk {i} due to end offset ({end_offset_ms}ms) "
                f"<= start offset ({start_offset_ms}ms)."
            )
            continue

        # Revert filename format to match test expectation
        ...
        try:
            ...
            logger.error(
                f"Failed to trim chunk {i} for {audio_file_path.name}. Skipping chunk. "
                f"Error: {err_msg}"
            )
            # raise # Uncomment to stop processing on first trim error
        except Exception as e:  # pylint: disable=broad-exception-caught
            logger.error(
                f"Unexpected error processing chunk {i} ({start_offset_ms}ms - {end_offset_ms}ms): "
                f"{e}"
            )
            # raise # Uncomment to stop processing on first trim error
